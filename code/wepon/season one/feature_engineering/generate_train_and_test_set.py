...
def get_label(s):
    ...
    if s[0] == 'null':
        return 0
    elif (date(int(s[0][0:4]), int(s[0][4:6]), int(s[0][6:8])) - date(
            int(s[1][0:4]), int(s[1][4:6]), int(s[1][6:8]))).days <= 15:
        return 1
    else:
        return -1


coupon3 = pd.read_csv('data/coupon3_feature.csv', keep_default_na=False)
merchant3 = pd.read_csv('data/merchant3_feature.csv', keep_default_na=False)
user3 = pd.read_csv('data/user3_feature.csv', keep_default_na=False)
user_merchant3 = pd.read_csv('data/user_merchant3.csv', keep_default_na=False)
other_feature3 = pd.read_csv('data/other_feature3.csv', keep_default_na=False)
...
coupon2 = pd.read_csv('data/coupon2_feature.csv', keep_default_na=False)
merchant2 = pd.read_csv('data/merchant2_feature.csv', keep_default_na=False)
user2 = pd.read_csv('data/user2_feature.csv', keep_default_na=False)
user_merchant2 = pd.read_csv('data/user_merchant2.csv', keep_default_na=False)
other_feature2 = pd.read_csv('data/other_feature2.csv', keep_default_na=False)
dataset2 = pd.merge(coupon2, merchant2, on='merchant_id', how='left')
dataset2 = pd.merge(dataset2, user2, on='user_id', how='left')
dataset2 = pd.merge(
    dataset2, user_merchant2, on=['user_id', 'merchant_id'], how='left')
dataset2 = pd.merge(
    dataset2,
    other_feature2,
    on=['user_id', 'coupon_id', 'date_received'],
...
dataset2['label'] = dataset2.date.astype(
    'str') + ':' + dataset2.date_received.astype('str')
dataset2.label = dataset2.label.apply(get_label)
dataset2.drop(
    [
        'merchant_id', 'day_of_week', 'date', 'date_received', 'coupon_id',
        'coupon_count'
    ],
    axis=1,
    inplace=True)
...
coupon1 = pd.read_csv('data/coupon1_feature.csv', keep_default_na=False)
merchant1 = pd.read_csv('data/merchant1_feature.csv', keep_default_na=False)
user1 = pd.read_csv('data/user1_feature.csv', keep_default_na=False)
user_merchant1 = pd.read_csv('data/user_merchant1.csv', keep_default_na=False)
other_feature1 = pd.read_csv('data/other_feature1.csv', keep_default_na=False)
dataset1 = pd.merge(coupon1, merchant1, on='merchant_id', how='left')
dataset1 = pd.merge(dataset1, user1, on='user_id', how='left')
dataset1 = pd.merge(
    dataset1, user_merchant1, on=['user_id', 'merchant_id'], how='left')
dataset1 = pd.merge(
    dataset1,
    other_feature1,
    on=['user_id', 'coupon_id', 'date_received'],
